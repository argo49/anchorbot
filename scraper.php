...
$request = $WebSearchURL . urlencode( '\'' . urldecode($query). '\'');
...
/	//the bingSearchAndScrape function urlencodes the search terms, so we don't do it here
	...
	foreach($newsOutlets as $newsOutlet){
		...
		$scrapeResult = bingSearchAndScrape($searchTerm,$newsOutlet);
...
if(filter_var($searchTerm, FILTER_VALIDATE_URL) === TRUE){
